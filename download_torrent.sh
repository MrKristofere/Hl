...
download_magnet_link() {
    ...
    if [ -n "$magnet_link" ]; then
        ...
        aria2c -d "$output_dir" --out "$downloaded_file" --enable-dht=true --continue=true --dht-file-path="/home/runner/.cache/aria2/dht.dat" "$magnet_link" || { echo "Ошибка загрузки файла!"; exit 1; }
    else
